<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [speech-to-text](./speech-to-text.md) &gt; [STTStream](./speech-to-text.sttstream.md) &gt; [(constructor)](./speech-to-text.sttstream._constructor_.md)

## STTStream.(constructor)

Constructs a new instance of the `STTStream` class

<b>Signature:</b>

```typescript
constructor(audioFilename: STTStream["audioFilename"], textFilename: STTStream["textFilename"], options: STTStreamOptions);
```

## Parameters

|  Parameter | Type | Description |
|  --- | --- | --- |
|  audioFilename | STTStream\["audioFilename"\] | Path to audio file |
|  textFilename | STTStream\["textFilename"\] | Path to text file |
|  options | [STTStreamOptions](./speech-to-text.sttstreamoptions.md) | Options |

